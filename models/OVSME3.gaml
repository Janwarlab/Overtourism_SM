...
// Lista de Playas
list<MZ> SITSPBR <- MZ where (each.AT=1 and each.Playa=1 and each.EPP_t>=espput_pc);
...
list<MZ> SITSHM <- MZ where (each.AT=1 and each.EPCH=1 and each.EPP_t>=espput_pc);
list<MZ> SITSH <- SITSHM + SITSHM + SITSHM;

// Lista de sitios comerciales (como centros comerciales)
list<MZ> SITSCOM <- MZ where (each.AT=1 and each.Comercial=1 and each.EPP_t>=espput_pc);

// Lista de Sitios Naturales
list<MZ> SITSSN <- MZ where (each.AT=1 and each.SN=1 and each.EPP_t>=espput_pc);
list<MZ> SNAT <- SITSSN + SITSSN;

// Parques
list<MZ> Parques <- MZ where (each.AT=1 and each.Parque=1 and each.EPP_t>=espput_pc);
...
living_place <- host;
SIT_placeD <- one_of (MZ where (each.AT=1 and each.EPP_t>=espput_pc));
objective <- "resting";
...
rgb color <- #red;
MZ living_place <- nil;
MZ SIT_placeD <- one_of (MZ where (each.AT=1 and each.EPP_t>=espput_pc));
